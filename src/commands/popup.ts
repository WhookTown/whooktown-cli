...
import type { LayoutDB, Layout, Building } from '@whooktown/sdk';
 ...
 */
async function findBuildingInLayouts(
  ...
  for (const layoutDB of layouts) {
    const layoutData = layoutDB.data as Layout;
...
const labelsCommand = new Command('labels')
  ...
  .action(async (layoutId: string, options: { on?: boolean; off?: boolean }) => {
    try {
    ...
    async (
      ...
      try {
        ...
        if (!hasOption) {
          ...
          console.error(
            'Use: --description, --tags, --notes, --clear-description, --clear-tags, --clear-notes'
          );
          process.exit(1);
        }

        const client = createClient();
        const { layoutDB, layoutData, building } = await findBuildingInLayouts(
          client,
          buildingId
        );
...
const getCommand = new Command('get')
  ...
  .action(async (buildingId: string, options: { format: string }) => {
    try {
      const client = createClient();
      const { layoutDB, layoutData, building } = await findBuildingInLayouts(
        client,
      ...
      // Text format
      const label = (text: string) => chalk.bold.cyan(text);
      ...
      console.log(
        `  ${label('Description:')} ${building.description || chalk.gray('-')}`
      );
      console.log(
        `  ${label('Tags:')}        ${building.tags?.length ? building.tags.join(', ') : chalk.gray('-')}`
      );
...
const listCommand = new Command('list')
  ...
  .action(
    async (layoutId: string, options: { format: string; tags?: string }) => {
      try {
        const client = createClient();
        const layouts = await client.ui.getLayouts();
        ...
        if (options.tags) {
          ...
          buildings = buildings.filter((b) =>
            b.tags?.some((t) => filterTags.includes(t.toLowerCase()))
          );
        }
